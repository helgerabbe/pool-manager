 ...
 * pullMoodleAbgaben
 *
 * Holt die Moodle-Abgabe-Adressen aus dem Austauschordner und trägt sie in die
 * zugehörigen Aufgaben ein. Das Gegenstück zu pullBrianUrls: Die Abgaben
 * entstehen beim Kursbau in Moodle, der Pool-Manager erfährt die Adresse erst
 * hierüber.
 *
 * Ablageort und Format: kurse/<slug>/moodle/<YYYY-MM-DD>.json ("moodle-abgaben-1")
 *
 * Zwei Betriebsarten (wie pullBrianUrls):
...
import {
  getMoodleAbgabeOrdner,
  parseMoodleAbgaben,
  findeAufgabe,
  baueAufgabenUpdate,
} from '../../shared/moodleAbgabeRueckmeldung.js';

async function verarbeiteEinheit(base44, token, einheit, jetzt) {
  const slug = getKursSlug(einheit);
  const ordner = getMoodleAbgabeOrdner(slug);
  ...
  if (!datei) {
    return {
      ...
      slug,
      gefunden: false,
      hinweis: `Im Repository liegen noch keine Moodle-Adressen unter ${ordner}/.`,
    };
  }
  ...
  const { eintraege, warnungen } = parseMoodleAbgaben(roh, datei.path);
  ...
  for (const eintrag of eintraege) {
    ...
    // Mehrere Abgabe-Schritte derselben Aufgabe: aufeinander aufbauend
    // zusammenführen, sonst überschreibt die zweite Adresse die erste.
    const schon = updates.findIndex((u) => u.id === update.id);
    if (schon >= 0) {
      const vorher = updates[schon];
      // Beide Updates betreffen dieselbe Schrittfolge — das zweite muss auf dem
      // Ergebnis des ersten aufbauen, nicht auf dem Bestand.
      if (update.sequenz_schritte && vorher.sequenz_schritte) {
        const zusammen = baueAufgabenUpdate(
          eintrag,
          { ...aufgabe, sequenz_schritte: vorher.sequenz_schritte },
          jetzt
        );
        updates[schon] = { ...vorher, ...(zusammen || {}) };
      } else {
        updates[schon] = { ...vorher, ...update };
      }
    } else {
      updates.push(update);
    }
...
export default async function (req) {
  try {
    ...
    if (body?.alle === true) {
      let alsAutomation = istAutomationAufruf(req);
      if (!alsAutomation) {
