 ...
 * Hinweis: Das Base44-REST-SDK unterstützt keine echten DB-Transaktionen; die Operationen werden deshalb
 * in sicheren Batches ausgeführt. @MIGRATION_NOTE Supabase: als transaktionale RPC/Stored Procedure umsetzen.
 *
 * Payload:
 * {
 *   einheit_id: string,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const currentEinheitVersion = Number(einheitRecord?.version || 1);
    ...
    const themenfeldMap = {};
    const existingThemenfelder = spalten.filter((spalte) => spalte.themenfeldId);
    const newThemenfelder = spalten.filter((spalte) => !spalte.themenfeldId);

    await runBatched(
      existingThemenfelder.map((spalte) => {
        const index = spalten.indexOf(spalte);
    ...
    const createdThemenfelder = await Promise.all(
      newThemenfelder.map((spalte) => {
        const index = spalten.indexOf(spalte);
        const bearbeitungsmodus = modesMap?.[spalte.id] || 'offen';
    ...
    newThemenfelder.forEach((spalte, index) => {
    ...
    const existingPackets = await listAllByFilter(base44.entities.Lernpakete, { einheit_id });
    ...
    for (const [spalteId, pakete] of Object.entries(paketeMap)) {
      ...
      for (let i = 0; i < pakete.length; i++) {
        ...
        if (paket.isNew) {
          ...
          packetUpdates.push(() => base44.entities.Lernpakete.update(paket.id, {
            themenfeld_id: themenfeldId,
            reihenfolge_nummer: i + 1,
          }));
        }
