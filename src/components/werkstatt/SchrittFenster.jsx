...
export default function SchrittFenster({
  ...
  // Zähler, um die Vorschau von Hand neu aufzubauen. Der iframe zeigt in
          ...
          <div className="flex flex-col min-h-0 rounded-xl border-2 border-slate-300 bg-white overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-100 border-b border-slate-200 shrink-0">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="ml-auto h-7 gap-1.5 text-xs text-slate-600"
                onClick={() => setVorschauZaehler((z) => z + 1)}
                title="Vorschau neu aufbauen, falls sie nicht mitgekommen ist"
              >
                <RefreshCw className="w-3 h-3" /> Vorschau aktualisieren
              </Button>
            </div>
            <div className="flex-1 min-h-0 bg-background">
              {/* Nur EIN Schritt: Die Aufgabenstellung der Folge wäre hier
                  irreführend — der Schritt trägt seine eigene. */}
              {/* key: erzwingt einen frischen Renderer je Schritt und bei
                  jedem Klick auf "Vorschau aktualisieren". Ohne das zeigte die
                  Vorschau beim Wechsel auf einen neuen Schritt noch den Stand
                  des vorigen. */}
              <AufgabensequenzSeite
                key={`${entwurf.id}-${vorschauZaehler}`}
