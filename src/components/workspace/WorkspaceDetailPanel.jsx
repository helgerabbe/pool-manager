...
function StepEmptyState({ icon: Icon, title, description, actionLabel, onAction }) {
  return (
...
// ── Wizard-Schritt-Hinweis ────────────────────────────────────────────────────

function WizardHint({ message }) {
  return (
...
function EinheitPanel({ einheit, lernpakete, lernziele, aufgaben, kannBearbeiten, onNavigate, onEdit }) {
  const completePakete = lernpakete.filter(p => {
    const ziele = lernziele.filter(lz => lz.lernpaket_id === p.id);
    return ziele.length > 0 && ziele.every(lz =>
      aufgaben.some(a => a.lernpaket_id === p.id && a.lernziel_id === lz.id)
    );
  });
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        {kannBearbeiten && (
          <Button variant="outline" size="sm" onClick={onEdit} className="gap-2">
            <Edit className="w-4 h-4" /> Einheit bearbeiten
          </Button>
        )}
      </div>
      ...
      {lernpakete.length > 0 && (
        <div className="p-4 rounded-xl border bg-card">
          <p className="text-sm font-medium mb-2">Alignment-Status</p>
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-muted rounded-full h-2">
              <div
                className="bg-green-500 h-2 rounded-full transition-all"
                style={{ width: lernpakete.length > 0 ? `${(completePakete.length / lernpakete.length) * 100}%` : '0%' }}
              />
            </div>
            <span className="text-xs text-muted-foreground shrink-0">
              {completePakete.length}/{lernpakete.length} vollständig
            </span>
          </div>
        </div>
      )}
      ...
      {lernpakete.length === 0 && (
        <WizardHint message="Legen Sie zunächst ein Lernpaket an, um mit der Planung zu beginnen." />
      )}

      {/* Pakete-Übersicht */}
      <div className="space-y-2">
        {lernpakete.map(paket => {
          const isComplete = completePakete.some(p => p.id === paket.id);
          return (
            <button
              key={paket.id}
...
function LernpaketPanel({ paket, lernziele, aufgaben, kannBearbeiten, onNavigate, onNewLernziel, onDelete }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground flex items-center gap-2">
            ...
            {paket.geschaetzte_dauer_minuten} Minuten geschätzte Dauer
          </p>
        </div>
        {kannBearbeiten && (
      ...
      {paketZiele.length === 0 ? (
        <StepEmptyState
          ...
          description="Definieren Sie zuerst Lernziele für dieses Paket, bevor Sie Aufgabenbausteine anlegen können."
          actionLabel={kannBearbeiten ? "Erstes Lernziel anlegen" : undefined}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Badge className={`mt-1 text-[10px] ${ebeneColors[lz.anforderungsebene] || ''}`}>
...
function LernzielPanel({ lernziel, paketId, aufgaben, userEmail, kannBearbeiten, istAdmin, onNewAufgabe, onDelete }) {
  ...
  const handleEditClose = async (data) => {
    if (data && editAufgabe) {
      await updateAufgabe.mutateAsync({ id: editAufgabe.id, data });
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <Badge className={`mb-2 ${ebeneColors[lernziel.anforderungsebene] || ''}`}>
            {lernziel.anforderungsebene}
          </Badge>
        ...
        {kannBearbeiten && (
          <Button variant="ghost" size="icon" onClick={onDelete} title="Lernziel löschen">
            <Trash2 className="w-4 h-4 text-destructive" />
          </Button>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">Aufgabenbausteine für dieses Lernziel</span>
      </div>
      ...
      {lzAufgaben.length === 0 ? (
        <StepEmptyState
          ...
          description={kannBearbeiten
            ? "Erstellen Sie jetzt den ersten Aufgabenbaustein für dieses Lernziel."
            : "Für dieses Lernziel gibt es noch keine Aufgabenbausteine."}
          actionLabel={kannBearbeiten ? "Ersten Baustein anlegen" : undefined}
...
/**
 * WorkspaceDetailPanel — rendert das zum selectedNode passende Detail-Panel.
 *
 * Das ist der zentrale Conditional-Rendering-Knoten des Workspaces.
 * Jeder selectedNode.type wird exakt auf ein Panel gemappt.
 */
export default function WorkspaceDetailPanel({
  ...
  // Wenn "new-*" Node ausgewählt → Formular direkt öffnen
  React.useEffect(() => {
  ...
  const createAufgabe = useMutation({
    mutationFn: (data) => {
      const clean = {
        ...data,
        lernpaket_id: selectedNode?.paketId,
        lernziel_id: selectedNode?.lernzielId,
      };
  ...
  // ── Conditional Rendering nach Node-Typ ──
    ...
    return (
      <>
        ...
        <AufgabenbausteinForm
          ...
          onSubmit={(data) => createAufgabe.mutate({
            ...data, lernpaket_id: paketId, lernziel_id: lernziel.id,
          })}
          lernziele={[lernziel]}
        />
      </>
  ...
  // "Neu"-Formular-Nodes: Modals sind oben via useEffect gesteuert
  if (type === 'new-lernpaket') {
    return (
      <>
        <StepEmptyState icon={Layers} title="Neues Lernpaket" description="Das Formular öffnet sich automatisch." />
        <LernpaketForm
  ...
  if (type === 'new-lernziel') {
    return (
      <>
        <StepEmptyState icon={Target} title="Neues Lernziel" description="Das Formular öffnet sich automatisch." />
        <LernzielForm
          open={lernzielFormOpen}
          onOpenChange={setLernzielFormOpen}
  ...
  if (type === 'new-aufgabe') {
    ...
    return (
      <>
        <StepEmptyState icon={Puzzle} title="Neuer Aufgabenbaustein" description="Das Formular öffnet sich automatisch." />
        <AufgabenbausteinForm
          open={aufgabeFormOpen}
          onOpenChange={setAufgabeFormOpen}
