        ...
        <div className="pt-3" ref={containerRef}>
          ...
          <article className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="px-5 py-4 bg-gradient-to-br from-blue-50 to-indigo-50 border-b border-blue-100">
              ...
              {bank.length === 0 ? (
                <p className="text-sm text-slate-500 italic">Alle Wörter sind eingesetzt – jetzt überprüfen!</p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {bank.map(word => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium border-2 cursor-grab active:cursor-grabbing transition-all hover:scale-105 hover:shadow-md ${
              ...
              {selectedWordId && (
                <p className="text-[11px] text-blue-700 mt-2 font-medium">
                  Tippe auf eine Lücke, um „{bank.find(w => w.id === selectedWordId)?.text || ''}" einzusetzen.
                </p>
              )}
            </div>

            {/* Text mit Lücken */}
            <div className="px-5 py-5 leading-loose text-[15px] text-slate-800">
              {tokens.map((tok, i) => {
                if (tok.type === 'text') {
                ...
                return (
                  <span
                    ...
                    className={`inline-flex items-center justify-center gap-1 mx-0.5 px-3 py-0.5 min-w-[90px] rounded-md border-2 border-dashed text-sm font-semibold cursor-pointer transition-all ${stateClass}`}
                  >
                    {word ? (
                      <>
                        <span>{word.text}</span>
                        {showFeedback && correct === true && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />}
                        {showFeedback && correct === false && <XCircle className="w-3.5 h-3.5 text-red-600" />}
                      </>
                    ) : (
                      <span className="text-xs opacity-60">______</span>
                    )}
                  </span>
                );
              })}
            ...
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-2">
              <Button variant="ghost" size="sm" onClick={handleReset} className="gap-1.5 text-slate-600">
