 ...
 * TestBody.jsx
 *
 * Interaktiver Schüler-Body für die Abschluss-Aktivität "Test".
 * Erwartet field_values mit:
 *   - aufgabentext (optional): einleitender Hinweistext
 *   - questions: Array von Fragen (single-/multi-choice, true_false, solution_word/text)
 *
...
function isCorrectFlag(a) { return a?.isCorrect === true || a?.correct === true; }
...
function QuestionBlock({ q, index, value, onChange, checked }) {
  ...
  let correct = null;
  if (checked) {
    if (type === 'true_false') correct = value === q.correctAnswer;
    else if (type === 'solution_word' || type === 'text') correct = String(value || '').trim().toLowerCase() === String(q.expectedAnswer || '').trim().toLowerCase();
    else if (isMulti) {
  ...
  return (
    <div className={`rounded-lg border p-3 ${
      checked ? (correct ? 'border-emerald-300 bg-emerald-50/50' : 'border-red-300 bg-red-50/50') : 'border-slate-200 bg-white'
    }`}>
      <div className="flex items-start gap-2 mb-2">
        ...
        {checked && (correct ? <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" /> : <XCircle className="w-5 h-5 text-red-600 shrink-0" />)}
      </div>
      ...
      <div className="pl-6 space-y-1.5">
        {type === 'true_false' && (
          <div className="flex gap-2">
            {[true, false].map(v => (
              <button key={String(v)} disabled={checked} onClick={() => onChange(v)}
                className={`px-3 py-1.5 rounded-md border text-sm font-medium transition-colors ${
                  value === v ? 'border-blue-500 bg-blue-100 text-blue-900' : 'border-slate-300 bg-white hover:border-blue-300'
        ...
        {(type === 'solution_word' || type === 'text') && (
          <Input value={value || ''} onChange={(e) => onChange(e.target.value)} disabled={checked} placeholder="Antwort eingeben…" className="h-8 text-sm" />
        )}

        {type !== 'true_false' && type !== 'solution_word' && type !== 'text' && answers.length > 0 && (
          answers.map((a, ai) => {
            const isChosen = isMulti ? Array.isArray(value) && value.includes(ai) : value === ai;
            const showAsCorrect = checked && isCorrectFlag(a);
            const showAsWrong = checked && isChosen && !isCorrectFlag(a);
            return (
              <label key={ai} className={`flex items-center gap-2 px-2 py-1.5 rounded-md border text-sm cursor-pointer transition-colors ${
                ...
                <input type={isMulti ? 'checkbox' : 'radio'} name={`q-${q.id || index}`} checked={isChosen} disabled={checked}
                  onChange={() => {
                    if (isMulti) {
...
export default function TestBody({ fieldValues = {} }) {
  ...
  const computeScore = () => {
    ...
    questions.forEach((q, i) => {
      ...
      if (q.type === 'true_false') { if (v === q.correctAnswer) s++; }
        ...
        } else if (typeof v === 'number' && correctSet.has(v)) s++;
      }
    });
    return s;
  };

  const score = useMemo(() => checked ? computeScore() : 0, [checked, answers, questions]);
  ...
  return (
    <div ref={containerRef} className="bg-white h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-3">
        {questions.length === 0 ? (
          ...
          <QuestionBlock key={q.id || i} q={q} index={i} value={answers[i]}
      ...
      <div className="px-3 py-1.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-2 shrink-0">
        ...
        <Button size="sm" onClick={handleCheck} disabled={!allAnswered || checked}
